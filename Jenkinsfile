    ...
    //     }
	 stage('Build') {
        ...
        stage('Deploy to Cloudhub'){
        	steps {
				 withCredentials([file(credentialsId: 'settings', variable: 'settings')]){
        	    	sh "mvn -f pom.xml -s $settings package deploy -DskipTests -Dusername=bejoy_njc -Dpassword=Mule1234 -DapplicationName=jenkineprojectnew -Denvironment=Sandbox -DmuleDeploy -Dcloudhub.region=us-east-2 -Danypoint.platform.client_id=629b248d8a66690637c2a9d79070326781f282fe -Danypoint.platform.client_secret=sds -Dsecure.key=mule"
